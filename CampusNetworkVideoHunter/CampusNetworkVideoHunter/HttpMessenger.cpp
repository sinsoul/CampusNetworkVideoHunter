...
bool HttpMessenger::CreateAndSendRequest(char *RequestType,char *ResourcePath,char *Host,char *PostData,bool OnlyGetLength,char *Save2File)
{
	...
	m_RequestHeader=m_RequestHeader+"User-Agent: Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1)\r\n";
...
//	Msg("总长度:%d\r\n",m_ContentLength);
	HANDLE hSMplayer=INVALID_HANDLE_VALUE;
	if (Save2File!=NULL)
	{
		hSMplayer=CreateFile(Save2File,GENERIC_WRITE|GENERIC_READ,0,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);
	...
	while(m_ContentLength>n_recvd)
	{
		...
		ProgressMsg("Downloading:%.2lf%%",n_recvperc*100);
		...
		if (hSMplayer!=INVALID_HANDLE_VALUE)
		{
			...
			if(!WriteFile(hSMplayer,LastResponse, recvstatus,&dwNumberOfBytesWritten, NULL))
			{
